...
export class BatchEnhancedProcessor {
  ...
  async processTenements(
    ...
    console.log(`🚀 Starting batch processing of ${tenementNumbers.length} tenements`);
    console.log(`📊 Batch size: ${batchSize}, Delay: ${delayBetweenRequests}ms, Max retries: ${maxRetries}`);
    ...
    for (let i = 0; i < tenementNumbers.length; i += batchSize) {
      ...
      console.log(`\n📦 Processing batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(tenementNumbers.length / batchSize)}`);
    ...
    console.log(`✅ Successful: ${successful}/${tenementNumbers.length}`);
    console.log(`❌ Failed: ${failed}/${tenementNumbers.length}`);
    console.log(`⏱️ Total time: ${(processingTime / 1000 / 60).toFixed(2)} minutes`);
    console.log(`📊 Data quality score: ${dataQualityReport.dataCompletenessScore.toFixed(1)}%`);
  ...
  private async processSingleTenement(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        console.log(`📋 [${index + 1}/${total}] Processing ${tenementNumber} (attempt ${attempt}/${maxRetries})`);
        ...
        if (validateData) {
          ...
          if (!validationResult.isValid) {
            console.warn(`⚠️ Data validation warnings for ${tenementNumber}:`, validationResult.warnings);
          }
        }
        ...
        if (saveToDatabase) {
          ...
          if (!stored) {
            console.warn(`⚠️ Failed to store ${tenementNumber} in database, but continuing...`);
          }
        }
        ...
        console.log(`✅ [${index + 1}/${total}] Successfully processed ${tenementNumber} in ${processingTime}ms`);
        console.log(`   📊 Found: ${tenementData.sites.length} sites, ${tenementData.projects.length} projects, ${tenementData.production.length} production records`);
        
        return { success: true, data: tenementData };

      } catch (error) {
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
        console.error(`❌ [${index + 1}/${total}] Attempt ${attempt}/${maxRetries} failed for ${tenementNumber}: ${errorMessage}`);
  ...
  async processHolderTenements(
    ...
    if (tenementNumbers.length === 0) {
      console.log(`❌ No tenements found for holder: ${holderName}`);
      return {
        totalProcessed: 0,
        successful: 0,
        failed: 0,
    ...
    console.log(`📋 Found ${tenementNumbers.length} tenements for ${holderName}`);
